...
export default function StudentTimetable() {
  ...
  // Re-render every 30s so the "now happening" indicator stays fresh
  useEffect(() => {
    const i = setInterval(() => setNow(new Date()), 30_000);
    return () => clearInterval(i);
  }, []);

  // Periodically reload today's schedule (every 2 min) so live edits show up
  const loadAll = async () => {
    setLoading(true);
    try {
      const [todayR, tomR, weekR] = await Promise.all([
        client.get('/schedule', { params: { when: 'today' } }).catch(() => ({ data: { items: [] } })),
        client.get('/schedule', { params: { when: 'tomorrow' } }).catch(() => ({ data: { items: [] } })),
        client.get('/schedule').catch(() => ({ data: { items: [] } })),
      ]);
      setTodayItems(todayR.data?.items || []);
      setTomorrowItems(tomR.data?.items || []);
      setWeekItems(weekR.data?.items || []);
  ...
  useEffect(() => { loadAll(); }, []);
  useEffect(() => {
    const i = setInterval(loadAll, 120_000);
    return () => clearInterval(i);
  }, []);

  // Recompute today's status against current `now` (so "now/next" updates in real time)
  const todayLive = useMemo(() => withLiveStatus(todayItems, now), [todayItems, now]);
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          ...
          <p style={{ color: 'var(--gray-400)', fontSize: '0.85rem', margin: '4px 0 0' }}>
            Live updates from your class teacher — see what's happening now, next, and all week.
          </p>
        </div>
      </div>

      {/* Now happening hero — only if not a holiday */}
      ...
      <div className="tabs">
        <button className={`tab ${tab === 'today' ? 'active' : ''}`} onClick={() => setTab('today')}>
          Today <span style={{ marginLeft: 4, fontSize: '0.7rem', padding: '2px 6px', background: 'var(--gray-100)', borderRadius: 10 }}>
            {todayHoliday ? '🎉' : todayItems.length}
          </span>
        </button>
        <button className={`tab ${tab === 'tomorrow' ? 'active' : ''}`} onClick={() => setTab('tomorrow')}>
          Tomorrow <span style={{ marginLeft: 4, fontSize: '0.7rem', padding: '2px 6px', background: 'var(--gray-100)', borderRadius: 10 }}>
            {tomorrowHoliday ? '🎉' : tomorrowItems.length}
          </span>
        </button>
        <button className={`tab ${tab === 'week' ? 'active' : ''}`} onClick={() => setTab('week')}>
          Full Week <span style={{ marginLeft: 4, fontSize: '0.7rem', padding: '2px 6px', background: 'var(--gray-100)', borderRadius: 10 }}>{weekItems.length}</span>
        </button>
      </div>
      ...
      {loading ? (
        ...
        todayHoliday
          ? <HolidayHero label="Today" holiday={todayHoliday} />
          : <DayList day={todayName} items={todayLive} emptyText="No periods scheduled today. Enjoy the day!" />
      ) : tab === 'tomorrow' ? (
        tomorrowHoliday
          ? <HolidayHero label="Tomorrow" holiday={tomorrowHoliday} />
          : <DayList day={tomorrowName} items={tomorrowItems} emptyText="No periods scheduled tomorrow." />
      ) : (
        <WeekView items={weekItems} todayName={todayName} weekHolidays={weekHolidays} />
      )}
    </div>
  );
}
...
function withLiveStatus(items: ScheduleItem[], now: Date): ScheduleItem[] {
  ...
  let nextSet = false;
  return items.map(it => {
    if (it.day !== today) return { ...it, status: 'scheduled' as const };
    const [sh, sm] = it.start_time.split(':').map(Number);
    const [eh, em] = it.end_time.split(':').map(Number);
    const st = sh * 60 + sm;
    const et = eh * 60 + em;
    const nowMin = now.getHours() * 60 + now.getMinutes();
    if (st <= nowMin && nowMin < et) return { ...it, status: 'now' as const };
    if (nowMin < st && !nextSet) { nextSet = true; return { ...it, status: 'next' as const }; }
    if (nowMin < st) return { ...it, status: 'upcoming' as const };
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            <div style={{ fontSize: '0.75rem', color: meta.color, fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase' }}>
              <FiPlay style={{ display: 'inline', marginRight: 4 }} /> Happening now • {meta.label}
            </div>
          ...
          {nextItem && (
            <div style={{
              ...
              border: '1px solid var(--gray-200)', minWidth: 180,
            }}>
  ...
  return (
    <div className="card" style={{ marginBottom: 12, padding: 14, background: '#f9fafb', border: '1px dashed var(--gray-200)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--gray-500)', fontSize: '0.88rem' }}>
        <FiCalendar /> All periods for today are done. See you tomorrow!
      </div>
    </div>
  );
}
...
function PeriodRow({ item }: { item: ScheduleItem }) {
  ...
  return (
    <div style={{
      ...
      borderLeft: `4px solid ${isNow ? meta.color : isNext ? meta.color : meta.color + '60'}`,
...
function HolidayHero({ label, holiday }: { label: string; holiday: any }) {
  return (
    <div className="card" style={{
...
// Build a lookup: day-of-week ("Monday"..) → holiday object (first one covering that day this week)

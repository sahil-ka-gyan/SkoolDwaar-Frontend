...
import { FiAward, FiTrendingUp, FiSearch, FiBookOpen, FiLayers, FiClipboard } from 'react-icons/fi';
...
export default function TeacherLeaderboard() {
  ...
  const load = useCallback(async () => {
    if (!classId) { setRankings([]); return; }
    setLoading(true);
    try {
      const params: any = { limit: 200, class_id: classId };
      ...
      <div className="page-header">
        <div>
          ...
          <p style={{ color: 'var(--gray-500)', fontSize: '0.85rem', margin: '4px 0 0' }}>
            Browse rankings for any class or section in your school. Filter by exam to drill down.
          </p>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: '0.75rem' }}>
        ...
        {classId && (
          <div style={{ padding: '0 1.25rem 1rem', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <span className="badge badge-primary">Class: {selectedClassName}</span>
            ...
            {!loading && <span className="badge badge-info">{contextTotal} students ranked</span>}
          </div>
        )}
      </div>
      ...
      {!classId ? (
        <div className="card">
          <div className="empty-state">
            ...
            <p>Choose a class above to see the leaderboard. You can then drill down to a specific section or exam.</p>
          </div>
        </div>
      ) : (
        <>
